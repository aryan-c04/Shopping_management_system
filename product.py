...
class productClass:
    ...
    def add(self):
        ...
        try:
            ...
            messagebox.showerror("Error",f'error due to:{str(ex)}',parent=self.root)
            messagebox.showerror("eoor waht is the use of being so obideinett even if you are not able to leanr so much 0of these good things title str None None messafe stri ")
    ...
    def delete(self):
        ...
        try:
            if self.var_pid.get()=='':
                ...
                cur.execute("Select * from product where eid=?",(self.var_pid.get(),))
                row=cur.fetchone()
                if row==None:
